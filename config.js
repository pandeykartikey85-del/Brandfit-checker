...
const CONFIG_KEYS = {
  ...
  USER_GEMINI_KEY: 'bfc_user_gemini_key',
  SESSION_ID: 'bfc_session_id',
  USER_MODE: 'bfc_user_mode', // 'creator' | 'brand'
  USER_NAME: 'bfc_user_name'
};

function getUserGeminiKey() {
  return (localStorage.getItem(CONFIG_KEYS.USER_GEMINI_KEY) || '').trim();
}

function setUserGeminiKey(key) {
  if (typeof key === 'string' && key.trim()) {
    localStorage.setItem(CONFIG_KEYS.USER_GEMINI_KEY, key.trim());
  } else {
    localStorage.removeItem(CONFIG_KEYS.USER_GEMINI_KEY);
  }
}

function getUserMode() {
  return localStorage.getItem(CONFIG_KEYS.USER_MODE) || null;
}

function setUserMode(mode) {
  if (mode === 'creator' || mode === 'brand') {
    localStorage.setItem(CONFIG_KEYS.USER_MODE, mode);
  }
}

function getUserName() {
  return localStorage.getItem(CONFIG_KEYS.USER_NAME) || '';
}

function setUserName(name) {
  if (name && name.trim()) {
    localStorage.setItem(CONFIG_KEYS.USER_NAME, name.trim());
  }
}

function getAppDisplayName(mode = getUserMode()) {
  return mode === 'brand' ? 'Creator Fit' : 'Brand Fit';
}
...
function showSettingsModal() {
  ...
  const geminiKeyInput = document.getElementById('input-user-gemini-key');
  ...
  if (geminiKeyInput) {
    geminiKeyInput.value = getUserGeminiKey();
  }
...
function initSettings() {
  ...
  if (form) {
    form.addEventListener('submit', (e) => {
      ...
      const userGeminiKey = geminiKeyInput ? geminiKeyInput.value : '';

      saveConfigValues(supabaseUrl, supabaseKey);
      setUserGeminiKey(userGeminiKey);
      hideSettingsModal();
